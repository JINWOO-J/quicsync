...
- [x] 1. 의존성 추가 및 에러 타입 확장
  - [x] 1.1 Cargo.toml에 Phase 2/3 의존성 추가
...
- [x] 2. CLI 확장 (`src/cli.rs`)
  - [x] 2.1 새 CLI 플래그 파싱 구현
  ...
  - [x] 2.2 Property 테스트: --streams 옵션 파싱 정확성
    - **Property 2: --streams 옵션 파싱 정확성**
    - proptest로 1-64 범위 정수 → 파싱 성공, 0 또는 65+ → 오류 반환 검증
    - **Validates: Requirements 3.3, 3.4**

- [x] 3. Checkpoint - 의존성 및 CLI 확장 검증
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Transfer Metrics 및 Progress UI 구현
  - [x] 4.1 `src/metrics.rs` 신규 생성 - TransferMetrics 구현
  ...
  - [x] 4.2 `src/progress.rs` 신규 생성 - Progress UI 구현
  ...
  - [x] 4.3 Property 테스트: 바이트/속도 포맷 함수 정확성
...
- [x] 5. Stats Reporter 구현
  - [x] 5.1 `src/stats.rs` 신규 생성 - StatsReporter 구현
...
- [x] 7. Blake3 무결성 검사 구현
  ...
  - [x] 7.2 Property 테스트: Blake3 청크 encode/decode 라운드트립
  ...
  - [x] 7.3 Property 테스트: Blake3 손상 감지
...
- [x] 8. QUIC 세션 핀닝 구현
  - [x] 8.1 `src/quic.rs` 수정 - FingerprintVerifier 구현
  ...
  - [x] 8.2 Property 테스트: 인증서 지문 계산 및 hex 인코딩
...
- [x] 10. Checkpoint - 보안 계층 검증
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. 멀티스트림 병렬 전송 구현
  - [x] 11.1 `src/multi_stream.rs` 신규 생성 - MultiStreamManager 구현
  ...
  - [x] 11.2 Property 테스트: 스트림 결과 집계 정확성
...
- [x] 12. OpenTelemetry 트레이싱 구현
  - [x] 12.1 `src/telemetry.rs` 신규 생성 - TelemetryExporter 구현
...
- [x] 13. Session 통합 - 모든 컴포넌트 연결
  - [x] 13.1 `src/session.rs` 수정 - 메트릭 및 컴포넌트 주입
